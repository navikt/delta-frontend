...
export default function Calendar({ dateString, displayTime }: CalendarProps) {
  ...
  return (
    <div className="flex flex-col w-fit rounded border border-ax-border-neutral">
      <div className="bg-ax-danger-700 text-white flex justify-center">{month}</div>
      <div className="flex flex-col items-center">
        <div className="font-semibold text-3xl px-2">{day}</div>
        {displayTime && <div className="border-t px-2">{time}</div>}
      </div>
    </div>
  );
}
