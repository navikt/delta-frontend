...
import FestivalEventDescription from "./festivalEventDescription";
import {
  Alert,
  Button,
  Heading,
...
type FestivalEventDetailsProps = FullDeltaEvent & {
  user: User;
  hostname?: string;
  basePath: string;
  contactName: string;
...
export default function FestivalEventDetails({
  event,
  participants,
  hosts,
  categories,
  ...
  const eventUrl = `${hostname}/${basePath}/${event.id}`;
        ...
        <div className="flex flex-col ax-md:flex-row gap-4 items-start ax-md:items-center">
          ...
          {(function () {
            if (hosts.map((h) => h.email).includes(user.email)) {
              return (
                <Link
                  className="w-full h-fit aksel-button"
          ...
          >
            <Modal.Body>
              ...
              <p className="leading-normal mb-4">
                Arrangementet er dessverre fullt, men det er mulig at arrangøren
                har en venteliste eller planer om lignende arrangementer.
              </p>
              <p className="leading-normal mb-4">
                Meld interesse til arrangøren:
              </p>
            ...
            <Modal.Body>
              ...
              <BodyLong spacing>
                {isParticipant ? (
                  <>
                    {new Date(event.endTime) < new Date()
                      ? "Ønsker du å slette deltakelsen din? Hvis ja sletter Delta data lagret om deg knyttet til dette arrangementet."
                      : "Er du sikker på at du vil melde deg av? Dersom påmeldingsfristen er utløpt eller antallsbegrensing er nådd, kan du ikke melde deg på igjen."}
                  </>
                ) : (
        ...
        <div className="flex-grow flex flex-col gap-2 ax-md:w-2/4">
          ...
          >
            <Modal.Body>
              ...
              <Canvas
                text={`https://delta.nav.no/${basePath}/${event.id}`}
                options={{
